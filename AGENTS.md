...
- Worked examples from IATA's official documentation
...
npm run build            # Build to dist/ (runs copy-schemas first)
...
- `ApiTest.kt` - Validates all worked examples against schemas
...
./gradlew download                              # Download worked examples from IATA Confluence
./gradlew generateMetadata                      # Regenerate worked_examples.json from cached downloads
...
| `DownloadWorkedExamples.kt`  | Scrapes IATA Confluence wiki for worked example XMLs         |
| `GenerateExamplesMetadata.kt`| Regenerates `worked_examples.json` metadata from cached downloads with fuzzy matching |
...
# 2. Download worked examples from IATA wiki
./gradlew download

# 3. Validate examples
./gradlew validate

# 4. Copy schemas to backend
bash ../scripts/copy_schemas_to_backend.sh
```
...
raw_ndc_schemas/         →  ./gradlew flatten    →  ndc_schemas/
                                                      ├→ site/public/schemas/ (via copy-schemas)
                                                      └→ backend/src/main/resources/schemas/ (via copy script)

IATA Confluence wiki     →  ./gradlew download   →  worked_examples_downloads/ (gitignored cache)
                                                      ├→ site/public/worked_examples/
                                                      └→ site/src/data/worked_examples.json
```
...
**Steps:** Checkout → Setup Java 25 → Sync schemas → Run tests → Build shadow JAR → Install KraftKit → Deploy to Unikraft Cloud
...
**Triggers:** Push to `main` when `site/**` or `ndc_schemas/**` changes
...
- The backend also needs schemas copied to its resources (via `copy_schemas_to_backend.sh`)
