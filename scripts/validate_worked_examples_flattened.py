#!/usr/bin/env python3
"""
Script to validate IATA NDC Worked Examples XML files against flattened XSD schemas using xmllint.
...
FLATTENED_SCHEMAS_DIR = PROJECT_ROOT / "ndc_schemas"

# Version mapping: schema ID in URL -> flattened schema folder name
VERSION_MAP = {
    "2134": "21.3.5",
    "2135": "21.3.5",
    "2136": "24.1",
...
def extract_schema_info(xml_path: Path) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        result = subprocess.run(
            ["grep", "-o", r'xsd/[0-9]*/[A-Za-z0-9_-]*\.xsd', str(xml_path)],
            capture_output=True,
            text=True,
            timeout=5
        )
...
def main():
    print("=" * 100)
    print("IATA NDC Worked Examples - Schema Validation Report (using FLATTENED schemas)")
    print("=" * 100)
    print()
    ...
    if not FLATTENED_SCHEMAS_DIR.exists():
        print(f"Error: {FLATTENED_SCHEMAS_DIR} not found.")
        sys.exit(1)
    
    # Collect all XML files
    xml_files = sorted(WORKED_EXAMPLES_DIR.rglob("*.xml"))
    print(f"Found {len(xml_files)} XML files to validate")
    print(f"Using flattened schemas from: {FLATTENED_SCHEMAS_DIR}")
    print()
    ...
    for xml_path in xml_files:
        ...
        # Find schema file in flattened schemas
        msg_name = os.path.splitext(schema_filename)[0]
        # Remove IATA_ prefix if present to match folder naming convention
        if msg_name.startswith('IATA_'):
            folder_name = msg_name[5:]
        else:
            folder_name = msg_name
            
        schema_path = FLATTENED_SCHEMAS_DIR / schema_folder / folder_name / schema_filename
