<script lang="ts">
  ...
  $: {
      ...
      const hasComplexChildren = Array.from(node.children).some(c => ["complexType", "simpleType", "sequence", "choice", "all", "complexContent"].includes(c.localName));
  ...
  function checkExpandability(root: Element, defs: Record<string, Element>, visited = new Set<string>()): boolean {
       ...
       // 3. Inline Complex
       const inline = Array.from(root.children).find(c => c.localName === "complexType");
       if (inline) return hasElementChildren(inline, defs);
       
       return false;
  }

  function hasElementChildren(root: Element, defs: Record<string, Element>): boolean {
      // BFS/DFS to find first 'element'
      // We don't need full collection, just boolean
      const q = [root];
      let limit = 0;
      while(q.length > 0 && limit < 50) { // Safety limit
          const curr = q.shift()!;
          limit++;
                   ...
                   for (const cc of Array.from(child.children)) {
                       if (cc.localName === "extension" || cc.localName === "restriction") {
                           ...
                           if (base) {
                               ...
                               if (defs[`complexType:${base}`] && hasElementChildren(defs[`complexType:${base}`], defs)) return true;
                           }
                           q.push(cc);
                       }
                   }
  ...
  // Extract basic info
  let name = node.getAttribute("name") || node.getAttribute("ref") || "Anonymous";
  ...
  function resolve() {
    ...
    if (node.getAttribute("ref")) {
        ...
        if (distinct) {
            const refType = distinct.getAttribute("type");
             if (refType) {
                typeName = refType.split(":").pop();
             } else {
                 const inlineType = Array.from(distinct.children).find(c => c.localName === "complexType");
    ...
    if (typeName) {
        ...
        typeDefinition = definitions[`complexType:${typeName}`] || definitions[`simpleType:${typeName}`];
    }
  ...
  $: isMatch = searchQuery && (
       ...
       (documentation && documentation.toLowerCase().includes(searchQuery))
     );
  ...
  function handleChildSelect(e: CustomEvent) {
...
<div class="ml-4 border-l border-base-300 pl-2">
    <div 
        class="flex items-center group cursor-pointer rounded -ml-2 px-2 py-1 transition-all duration-200 border border-transparent
            {path === selectedPath ? 'bg-primary/10 border-primary/20 shadow-sm' : 'hover:bg-base-200/50'}
        ...
        <!-- Expand Button / Icon -->
        <button class="btn btn-xs btn-ghost btn-square w-5 h-5 min-h-0 mr-1 p-0 flex items-center justify-center opacity-70 hover:opacity-100 hover:bg-base-200 rounded-md transition-colors" 
            on:click|stopPropagation={() => { if(isExpandable) { expanded = !expanded; if(expanded) resolve(); } }}>
            {#if isExpandable}
                {#if expanded}
                    <!-- Chevron Down -->
            ...
            {:else}
                <!-- Leaf Dot -->
                 <svg xmlns="http://www.w3.org/2000/svg" class="h-1.5 w-1.5 opacity-30" viewBox="0 0 20 20" fill="currentColor">
        ...
        <!-- Icon -->
        <span class="mr-2 opacity-80 flex-none" title={typeName ? `Type: ${typeName}` : 'Element'}>
            {#if name.endsWith("RQ") || name.endsWith("RS")}
                 <!-- Message Icon -->
                 <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-purple-600" viewBox="0 0 20 20" fill="currentColor"><path d="M7 3a1 1 0 000 2h6a1 1 0 100-2H7zM4 7a1 1 0 011-1h10a1 1 0 110 2H5a1 1 0 01-1-1zM2 11a2 2 0 012-2h12a2 2 0 012 2v4a2 2 0 01-2 2H4a2 2 0 01-2-2v-4z" /></svg>
            {:else if typeName}
                 <!-- Typed Element Icon -->
                 <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-blue-500" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M2.5 3A1.5 1.5 0 001 4.5v.793c.026.009.051.02.076.032L7.674 8.51c.206.1.446.1.652 0l6.598-3.185A.755.755 0 0115 5.293V4.5A1.5 1.5 0 0013.5 3h-11zM15 6.913l-6.598 3.185a.755.755 0 01-.804 0L1 6.914V14.5a1.5 1.5 0 001.5 1.5h11a1.5 1.5 0 001.5-1.5V6.913z" clip-rule="evenodd" /></svg>
            {:else}
                 <!-- Generic Element Icon -->
                 <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-orange-500" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" /></svg>
            {/if}
        </span>

        <!-- Label & Badges -->
        <div class="flex-1 min-w-0 flex items-center {isMatch ? 'bg-yellow-100 text-yellow-900 rounded px-1' : ''} gap-2">
            <span class="font-medium text-sm text-base-content/90 truncate" title={path}>{name}</span>
            {#if typeName}
                <span class="hidden group-hover:inline-block text-[10px] text-base-content/40 font-mono transition-opacity whitespace-nowrap bg-base-200 px-1 rounded">{typeName}</span>
            {/if}
            
            <div class="flex items-center gap-1 ml-auto flex-none">
                 <!-- Cardinality Badges -->
                {#if minOccurs === "0"}
                    <span class="badge badge-xs badge-ghost font-mono text-[9px] uppercase tracking-wider text-base-content/50 border-base-300">OPT</span>
                {:else}
                    <span class="badge badge-xs badge-primary badge-outline font-mono text-[9px] uppercase tracking-wider font-bold">REQ</span>
                {/if}

                {#if maxOccurs === "unbounded" || (parseInt(maxOccurs) > 1)}
                    <span class="badge badge-xs badge-neutral font-mono text-[9px]" title="List (Array)">LIST</span>
                {/if}
            </div>
        </div>
    </div>
    ...
    {#if expanded || searchQuery}
        <div class={!expanded && searchQuery ? 'hidden' : 'border-l border-base-200 ml-[0.35rem]'}>
